...
function renderMessages() {
  [...chatWindowEl.children].forEach(ch => {
  ...
  chat.messages.forEach(msg => {
    chatWindowEl.insertBefore(
      buildMessageEl(msg.text, msg.role, msg.time),
      typingEl
    );
  });
...
function uploadFile() {
  const fi = document.createElement('input');
    ...
    if (fi.files[0]) {
      addMessage(`📎 Файл: ${fi.files[0].name}`, 'user');
    }
  };
}
